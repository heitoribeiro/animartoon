...
function classifyFile(file){const m=file.name.match(/^(C\d{3}[A-Z]?)_(IMG|ANIM|FINAL)_v(\d+)\.(png|jpe?g|webp|mp4|mov)$/i);if(!m)return {...file,recognized:false};return {...file,recognized:true,sceneId:m[1].toUpperCase(),kind:m[2].toUpperCase(),version:Number(m[3]),ext:m[4].toLowerCase()}}
function summarizeFiles(files){const recognized=files.filter(f=>f.recognized),unrecognized=files.filter(f=>!f.recognized),grouped={};for(const f of recognized){const key=f.sceneId+'_'+f.kind;(grouped[key]||(grouped[key]=[])).push(f)}const duplicates=Object.entries(grouped).filter(([,arr])=>arr.length>1).map(([key,arr])=>({key,count:arr.length,latest:arr.slice().sort((a,b)=>b.version-a.version)[0]}));return {recognized,unrecognized,grouped,duplicates}}
