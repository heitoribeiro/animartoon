...
function imagePrompt(s){return 'Create a consistent stylized 3D animated keyframe for scene '+s.id+': '+s.title+'. Characters: '+(s.characters.join(', ')||'none')+'. Location: '+(s.location||'unspecified')+'. '+(s.action?'Action: '+s.action+'. ':'')+(s.camera?'Camera/framing: '+s.camera+'. ':'')+'Preserve character identity, costume, proportions, biblical-era continuity, cinematic family-animation lighting and composition. '+(s.dialogue?'This scene will later include dialogue: "'+s.dialogue+'".':'No spoken dialogue is required in the image.')}
function animationPrompt(s){const d=sceneDuration(s).toFixed(1);return 'Create a '+d+'-second animation from the supplied image for scene '+s.id+': '+s.title+'. '+(s.action?'Action: '+s.action+'. ':'')+(s.type==='Diálogo'?'Include natural synchronized spoken dialogue'+(s.dialogue?' exactly as follows: "'+s.dialogue+'".':' based on the supplied scene audio/script.')+' Add natural facial acting and lip synchronization.':'Do not add spoken dialogue. Animate character/environment movement naturally.')+(s.ambience?' Scene sound: '+s.ambience+'. ':' Add subtle natural scene ambience. ')+(s.camera?'Camera direction: '+s.camera+'. ':'')+'Preserve the exact characters, costume, setting and visual continuity.'}
...
async function analyzeSelectedVideo(){
 ...
 try{
  ...
  const picked=pickVisualCuts(samples,fraction,step),built=buildDetectedScenes(picked.cuts,duration,splitLong),p=project();
...
function applyTranscriptCues(cues,replace=false){
 ...
 for(const s of p.scenes){
  ...
  if(s.type==='A revisar'||s.type==='Ambiente/Narração')s.type='Diálogo'
 }
...
function applyBulkEdit(){
 ...
 const type=document.getElementById('bulkType')?.value||'',location=(document.getElementById('bulkLocation')?.value||'').trim(),characters=(document.getElementById('bulkCharacters')?.value||'').trim(),ambience=(document.getElementById('bulkAmbience')?.value||'').trim();
 let changed=0;
