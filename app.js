...
function filemap(){const p=project(),summary=fileSummary(p),i=p.scenes.filter(s=>s.image==='done').length,a=p.scenes.filter(s=>s.animation==='done').length,f=p.scenes.filter(s=>s.approved).length,b=[['Personagens',p.characters.length,p.characters.length||1],['Cenários',p.locations.length,p.locations.length||1],['Imagens',i,p.scenes.length||1],['Animações',a,p.scenes.length||1],['Cenas finais',f,p.scenes.length||1]],physical=[['Arquivos totais',(p.fileIndex||[]).length],['Reconhecidos',summary.recognized.length],['Fora do padrão',summary.unrecognized.length],['Grupos com versões',summary.duplicates.length]];return '<div class="grid">'+(p.storage==='Google Drive no computador'?localFolderControls(p):p.storage==='Google Drive Online'?onlineDriveControls(p):'')+((p.storage==='Google Drive no computador'||p.storage==='Google Drive Online')?autoSyncControls(p):'')+'<section class="card span2"><p class="eyebrow">MAPA DA PRODUÇÃO</p><h2>Progresso por categoria</h2>'+b.map(x=>'<div class="mapline"><div class="topline"><strong>'+x[0]+'</strong><span>'+x[1]+' / '+x[2]+'</span></div><div class="mini"><div style="width:'+Math.min(100,x[1]/x[2]*100)+'%"></div></div></div>').join('')+'</section><section class="card span2"><p class="eyebrow">MAPA FÍSICO</p><h2>Arquivos encontrados</h2><div class="audit">'+physical.map(x=>'<div class="panel"><span class="muted">'+x[0]+'</span><strong>'+x[1]+'</strong></div>').join('')+'</div></section>'+(missingChars.length||missingScenarios.length?'<section class="card span2"><p class="eyebrow">REFERÊNCIAS</p><h2>Assets sem arquivo detectado</h2><div class="pending">'+missingChars.map(id=>'<div><strong>'+id+'</strong><span>Personagem</span><span class="warn">sem referência</span></div>').join('')+missingScenarios.map(id=>'<div><strong>'+id+'</strong><span>Cenário</span><span class="warn">sem referência</span></div>').join('')+'</div></section>':'')+(summary.duplicates.length?'<section class="card"><p class="eyebrow">VERSÕES</p><h2>Arquivos versionados</h2><div class="pending">'+summary.duplicates.slice(0,12).map(x=>'<div><strong>'+x.key+'</strong><span>'+x.count+' versões</span><span class="ok">última v'+String(x.latest.version).padStart(2,'0')+'</span></div>').join('')+'</div></section>':'')+(summary.unrecognized.length?'<section class="card"><p class="eyebrow">NÃO RECONHECIDOS</p><h2>Fora do padrão</h2><div class="pending">'+summary.unrecognized.slice(0,12).map(x=>'<div><span>'+esc(x.name)+'</span><small class="muted">'+esc(x.path)+'</small></div>').join('')+'</div></section>':'')+'<section class="card span2"><p class="eyebrow">PADRÃO DE ARQUIVOS</p><h2>Nomenclatura esperada</h2><div class="folders"><div>01_PERSONAGENS/<strong>ABRAHAM_01_FRONT.png</strong></div><div>02_CENARIOS/<strong>CAMP_OASIS_01_BASE.png</strong></div><div>03_IMAGENS_CENAS/<strong>C001_IMG_v01.png</strong></div><div>04_ANIMACOES/<strong>C001_ANIM_v01.mp4</strong></div><div>07_CENAS_FINAIS/<strong>C001_FINAL_v01.mp4</strong></div></div></section></div>'}
