...
return '<div class="grid"><section class="card span2"><div class="top"><div><p class="eyebrow">PROJETO ATIVO</p><h2>'+esc(p.name)+'</h2></div><div class="percent">'+progress(p)+'%</div></div><div class="progress"><div style="width:'+progress(p)+'%"></div></div></section><section class="card span2"><div class="section-title"><div><p class="eyebrow">PRÓXIMA AÇÃO</p><h2>'+s.id+' — '+kind+'</h2><p class="muted">'+esc(s.title)+'</p></div><span class="badge accent">'+s.type+'</span></div><div class="scene-meta"><div><small>Intervalo</small><strong>'+fmt(s.start)+' → '+fmt(s.end)+'</strong></div><div><small>Duração</small><strong>'+dur.toFixed(1)+' s</strong></div><div><small>Personagens</small><strong>'+esc(s.characters.join(', ')||'—')+'</strong></div><div><small>Cenário</small><strong>'+esc(s.location||'—')+'</strong></div></div>'+(dur>max?'<div class="warning">Esta cena tem '+dur.toFixed(1)+' s. O perfil atual dos geradores indica até '+max+' s; considere dividir em duas partes.</div>':'')+'<div class="two"><div><span class="label">Nome sugerido</span><div class="panel copybox"><code>'+file+'</code><button class="btn" onclick="copy(\''+file+'\')">Copiar</button></div></div><div><span class="label">Prompt</span><div class="panel"><p class="prompt">'+esc(prompt)+'</p><button class="btn" onclick="copy('+JSON.stringify(prompt)+')">Copiar prompt</button></div></div></div><div class="easy-generator-action"><button class="btn primary big-action" onclick="sendSceneToPreferredGenerator(\''+s.id+'\')">Abrir gerador e preparar cena ↗</button><span class="muted">Um clique: abre o gerador preferido, copia o prompt e baixa a imagem de referência da cena.</span></div><details class="generator-details"><summary>Outros geradores e opções</summary><div class="tools">'+tt.map(t=>'<a class="'+(t.preferred?'preferred':'')+'" href="'+t.url+'" target="_blank">↗ '+t.name+(t.preferred?' • preferida':'')+'</a>').join('')+'</div></details><button class="btn" onclick="markNext()">✓ Marcar etapa como concluída</button></section></div>'}
