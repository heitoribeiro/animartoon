...
async function sendSceneToPreferredGenerator(sceneId){
 ...
 try{
  const src=await loadSceneThumbnail(p.id,s.id);
  if(src){
   const a=document.createElement('a');a.href=src;a.download=s.id+'_SOURCE_REF.jpg';document.body.appendChild(a);a.click();a.remove();
  }
 }catch(e){}
