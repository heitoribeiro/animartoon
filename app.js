...
function driveAnalysisPanel(p){
 ...
 const videos=driveVideoFiles();
 return '<section class="card span2"><div class="section-title"><div><p class="eyebrow">ANALISAR ARQUIVO DO GOOGLE DRIVE</p><h2>Vídeo direto da pasta do projeto</h2></div><span class="badge '+(driveToken?'ok':'warn')+'">'+(driveToken?'Drive conectado':'Drive desconectado')+'</span></div><p class="muted">A Animartoon lista os vídeos da pasta autorizada e envia ao analisador apenas o arquivo selecionado. O arquivo é processado temporariamente e descartado pelo serviço após a análise.</p><div class="actions"><button class="btn" onclick="refreshDriveForAnalysis()">Atualizar lista de vídeos</button></div><div class="drive-video-list">'+(videos.length?videos.map(f=>'<div class="panel drive-video-item"><div><strong>'+esc(f.name)+'</strong><small class="muted">'+esc(f.path||'')+(f.size?' • '+(f.size/1024/1024).toFixed(1)+' MB':'')+'</small></div><button class="btn primary" onclick="analyzeDriveFile(\''+f.driveId+'\')">Analisar com FFmpeg</button></div>').join(''):'<p class="muted note">Nenhum vídeo listado ainda. Conecte o Drive e clique em Atualizar lista de vídeos.</p>')+'</div></section>'
}
