...
function productionScenes(p=project()){return (p?.scenes||[]).filter(s=>s.type!=='Ignorar')}
function progress(p=project()){const scenes=productionScenes(p);if(!scenes.length)return 0;let n=scenes.reduce((a,s)=>a+(s.image==='done')+(s.animation==='done')+(s.approved?1:0),0);return Math.round(n/(scenes.length*3)*100)}
function nextScene(p=project()){const scenes=productionScenes(p);return scenes.find(s=>s.image!=='done'||s.animation!=='done'||!s.approved)||scenes[0]}
...
function imagePrompt(s){return 'OUTPUT FILE: '+expectedFile(s,'image')+'. Use the supplied source-frame image as the primary visual reference. Recreate the SAME visible characters, poses, composition, camera angle, environment and story moment as a high-quality stylized 3D biblical animated-film keyframe. Do not add or remove visible characters or modern objects. Scene '+s.id+': '+s.title+'. '+(s.characters.length?'Known characters: '+s.characters.join(', ')+'. ':'')+(s.location?'Location: '+s.location+'. ':'')+(s.action?'Action: '+s.action+'. ':'')+(s.camera?'Camera/framing: '+s.camera+'. ':'')+'Preserve identity, costume, proportions and continuity with adjacent scenes. '+(s.dialogue?'Context from the original audio: "'+s.dialogue+'".':'No spoken-text context is available.')}
function animationPrompt(s){const d=sceneDuration(s).toFixed(1),speech=s.type==='Diálogo'?('Include natural synchronized spoken dialogue'+(s.speaker?' spoken only by '+s.speaker:'')+(s.dialogue?' exactly as follows: "'+s.dialogue+'".':' based on the supplied script.')+' Lip-sync only the speaking character.'):s.type==='Narração'?('Use the following as OFF-SCREEN NARRATION / VOICE-OVER, with no character lip-sync: "'+(s.dialogue||'')+'".'):('Do not add spoken dialogue or voice-over.');return 'OUTPUT FILE: '+expectedFile(s,'animation')+'. Create a '+d+'-second animation from the supplied image for scene '+s.id+': '+s.title+'. '+(s.action?'Action: '+s.action+'. ':'')+speech+' '+(s.ambience?'Scene sound: '+s.ambience+'. ':'Add subtle natural scene ambience. ')+(s.camera?'Camera direction: '+s.camera+'. ':'')+(s.continuityNote?'Continuity: '+s.continuityNote+'. ':'')+'Preserve the exact visible characters, costume, setting, lighting and visual continuity. Do not invent new characters or objects.'}
...
function easyModePanel(p){
 ...
 '<div class="easy-footer"><button class="btn" onclick="setSimpleMode(false)">Mostrar ferramentas avançadas</button>'+(p.scenes?.length?'<button class="btn" onclick="setPage(\'Cenas\')">Ver cenas</button><button class="btn primary" onclick="setPage(\'Próxima ação\')">Iniciar produção</button>':'')+'</div></section>'
}
...
function contextualRefineScenes(p=project(),force=false){
 const scenes=p.scenes||[],stable=new Set(['Diálogo','Narração','Ambiente','Sem fala','Ignorar']);
 // First, handle obvious non-production material and silent transitions.
 for(let i=0;i<scenes.length;i++){
  const s=scenes[i],text=String(s.dialogue||'').trim(),dur=sceneDuration(s),conf=Number(s.classificationConfidence||0);
  if(s.manualType&&!force)continue;
  const atStart=Number(s.start||0)<0.15;
  const atEnd=p.duration&&Number(s.end||0)>Number(p.duration)-4;
  const weakThumb=['dark','low_info','review'].includes(s.thumbnailStatus);
  if(atStart&&dur<=12&&(conf<0.64||text.length<12||weakThumb)){
   s.type='Ignorar';s.speechType='none';s.classificationConfidence=0.9;s.classificationReason='abertura/fade inicial sem valor de produção';continue;
  }
  if(atEnd&&dur<=8&&(!text||weakThumb)){
   s.type='Ignorar';s.speechType='none';s.classificationConfidence=0.88;s.classificationReason='encerramento/fade sem valor de produção';continue;
  }
  if(dur<0.65&&!text){
   s.type='Ignorar';s.speechType='none';s.classificationConfidence=0.86;s.classificationReason='transição muito curta sem fala';continue;
  }
 }
 // Resolve ambiguous scenes using technical siblings and neighboring context.
 for(let i=0;i<scenes.length;i++){
  const s=scenes[i];if(s.type!=='A revisar'||(s.manualType&&!force))continue;
  const group=sourceGroupScenes(s,p).filter(x=>x!==s&&stable.has(x.type)&&x.type!=='Ignorar'&&Number(x.classificationConfidence||0)>=0.72);
  if(group.length){
   const counts={};group.forEach(x=>counts[x.type]=(counts[x.type]||0)+1);
   const best=Object.entries(counts).sort((a,b)=>b[1]-a[1])[0];
   if(best&&best[1]>=Math.ceil(group.length/2)){s.type=best[0];s.speechType=best[0]==='Diálogo'?'character':best[0]==='Narração'?'voice_over':'none';s.classificationConfidence=0.76;s.classificationReason='continuidade com subcenas do mesmo corte';continue}
  }
  const prev=scenes[i-1],next=scenes[i+1];
  if(prev&&next&&prev.type===next.type&&stable.has(prev.type)&&prev.type!=='Ignorar'&&Number(prev.classificationConfidence||0)>=0.7&&Number(next.classificationConfidence||0)>=0.7){
   s.type=prev.type;s.speechType=prev.speechType||'unknown';s.classificationConfidence=0.78;s.classificationReason='contexto consistente entre cenas vizinhas';continue
  }
  const text=String(s.dialogue||'').trim();
  if(!text){s.type='Ambiente';s.speechType='none';s.classificationConfidence=0.86;s.classificationReason='sem fala após análise contextual';continue}
 }
 const counts={'Diálogo':0,'Narração':0,'Ambiente':0,'Sem fala':0,'A revisar':0,'Ignorar':0};
 scenes.forEach(s=>counts[s.type]=(counts[s.type]||0)+1);
 p.classificationStats={...counts,updatedAt:Date.now()};
 p.contextRefinedVersion=2;p.contextRefinedAt=Date.now();
 return counts
}
function reviewSceneType(sceneId,type){
 const p=project(),s=p.scenes.find(x=>x.id===sceneId);if(!s)return;
 s.type=type;s.manualType=true;s.classificationConfidence=1;s.classificationReason='confirmado na revisão rápida';
 if(type==='Diálogo')s.speechType='character';else if(type==='Narração')s.speechType='voice_over';else s.speechType='none';
 contextualRefineScenes(p,false);save();render()
}
function reclassifyExistingScenes(force=false){
 const p=project();
 for(const s of p.scenes)applyAutoClassification(s,s.dialogue||'',force);
 const counts=contextualRefineScenes(p,force);save();render();toast('Classificação contextual atualizada • '+(counts['A revisar']||0)+' pendência(s)')
}
function applyTranscriptCues(cues,replace=false){
 ...
 p.transcriptSceneCount=count;p.transcriptAppliedAt=Date.now();p.classificationStats={...counts,updatedAt:Date.now()};contextualRefineScenes(p,false);save();render();toast(count+' cenas receberam fala e foram classificadas')
}
...
function nextView(){
 const p=project(),reviews=(p.scenes||[]).filter(s=>s.type==='A revisar');
 if(reviews.length){
  const s=reviews[0],remaining=reviews.length,transcript=String(s.dialogue||'').trim();
  return '<div class="grid"><section class="card span2"><div class="section-title"><div><p class="eyebrow">REVISÃO RÁPIDA</p><h2>Só '+remaining+' decisão'+(remaining===1?'':'ões')+' antes da produção</h2><p class="muted">A Animartoon resolveu o restante automaticamente. Classifique apenas os casos realmente ambíguos.</p></div><span class="badge warn">'+remaining+' pendente(s)</span></div><div class="quick-review"><img class="quick-review-thumb" data-scene-thumb="'+s.id+'" alt="Miniatura '+s.id+'"><div><h3>'+s.id+' — '+esc(s.title)+'</h3><p class="muted">'+fmt(s.start)+' → '+fmt(s.end)+' • '+sceneDuration(s).toFixed(1)+' s</p><div class="panel review-text">'+(transcript?esc(transcript):'<span class="muted">Nenhuma fala detectada.</span>')+'</div><p class="muted">Escolha apenas o que melhor descreve a cena:</p><div class="quick-choice"><button class="btn primary" onclick="reviewSceneType(\''+s.id+'\',\'Diálogo\')">Diálogo</button><button class="btn" onclick="reviewSceneType(\''+s.id+'\',\'Narração\')">Narração</button><button class="btn" onclick="reviewSceneType(\''+s.id+'\',\'Ambiente\')">Ambiente</button><button class="btn" onclick="reviewSceneType(\''+s.id+'\',\'Sem fala\')">Sem fala</button><button class="btn danger" onclick="reviewSceneType(\''+s.id+'\',\'Ignorar\')">Ignorar</button></div></div></div></section></div>'
 }
 const s=nextScene(p);
 if(!s)return '<section class="card empty"><h2>Nenhuma cena para produzir</h2><p class="muted">Prepare um vídeo na Importação.</p><button class="btn primary" onclick="setPage(\'Importação\')">Preparar vídeo</button></section>';
 const image=s.image!=='done',kind=image?'Criar imagem 3D':s.animation!=='done'?'Criar animação':'Aprovar cena',file=image?expectedFile(s,'image'):expectedFile(s,'animation'),prompt=image?imagePrompt(s):animationPrompt(s),dur=sceneDuration(s),scenes=productionScenes(p),done=scenes.filter(x=>x.approved).length;
 if(!image&&s.animation==='done'&&!s.approved){
  return '<div class="grid"><section class="card span2 production-assistant"><div class="section-title"><div><p class="eyebrow">CONFERÊNCIA RÁPIDA</p><h2>'+s.id+' — resultado recebido</h2><p class="muted">Só confirme se o resultado está aproveitável. Não é necessário editar nada aqui.</p></div><span class="badge ok">pronto</span></div><div class="assistant-layout"><img class="assistant-thumb" data-scene-thumb="'+s.id+'" alt="Referência '+s.id+'"><div class="assistant-main"><h3>'+esc(s.title)+'</h3><p class="muted">Arquivo recebido: '+esc(s.generatedAnimationName||'animação registrada')+'</p><button class="btn primary huge-action" onclick="markNext()">Aprovar e ir para a próxima cena ✓</button><button class="btn" onclick="editSceneByIdStage(\''+s.id+'\',\'animation\')">Quero gerar esta animação novamente</button></div></div></section></div>'
 }
 return '<div class="grid"><section class="card span2 production-assistant"><div class="section-title"><div><p class="eyebrow">ASSISTENTE DE PRODUÇÃO</p><h2>'+s.id+' — '+kind+'</h2><p class="muted">Cena '+(scenes.indexOf(s)+1)+' de '+scenes.length+' • '+done+' finalizada(s)</p></div><span class="badge accent">'+s.type+'</span></div><div class="assistant-layout"><img class="assistant-thumb" data-scene-thumb="'+s.id+'" alt="Referência '+s.id+'"><div class="assistant-main"><h3>'+esc(s.title)+'</h3><p class="muted">'+fmt(s.start)+' → '+fmt(s.end)+' • '+dur.toFixed(1)+' s</p>'+(s.dialogue?'<div class="panel assistant-script"><strong>'+(s.type==='Narração'?'Narração':'Fala')+'</strong><p>'+esc(s.dialogue)+'</p></div>':'')+'<div class="assistant-output"><span class="muted">Salvar resultado como</span><code>'+file+'</code></div><button class="btn primary huge-action" onclick="sendSceneToPreferredGenerator(\''+s.id+'\')">1. Preparar e abrir no gerador ↗</button><p class="muted center-note">Referência baixada + prompt copiado automaticamente.</p><label class="result-drop"><strong>2. Trazer o resultado para a Animartoon</strong><span>Arraste aqui ou clique para escolher o arquivo que o gerador baixou. Não precisa renomear.</span><input type="file" accept="'+(image?'image/*':'video/*')+'" onchange="acceptGeneratedResult(\''+s.id+'\',event)"></label><button class="btn done-action" onclick="markNext()">Pular recebimento e avançar manualmente</button></div></div><details class="generator-details"><summary>Ver prompt e opções avançadas</summary><div class="panel"><p class="prompt">'+esc(prompt)+'</p><button class="btn" onclick="copy('+JSON.stringify(prompt)+')">Copiar prompt</button></div></details></section></div>'
}
function production(){const p=project(),scenes=productionScenes(p);return '<section class="card"><div class="section-title"><div><p class="eyebrow">PIPELINE</p><h2>Produção por cena</h2></div><span class="badge">'+scenes.length+' cenas</span></div><div class="table">'+scenes.map(s=>'<div class="row"><strong>'+s.id+'</strong><span>'+esc(s.title)+'<small>'+sceneDuration(s).toFixed(1)+' s</small></span><button class="statusbtn '+(s.image==='done'?'ok':'warn')+'" onclick="toggleStatus(\''+s.id+'\',\'image\')">'+(s.image==='done'?'✓':'○')+' Imagem</button><button class="statusbtn '+(s.animation==='done'?'ok':'warn')+'" onclick="toggleStatus(\''+s.id+'\',\'animation\')">'+(s.animation==='done'?'✓':'○')+' Animação</button><button class="statusbtn '+(s.approved?'ok':'warn')+'" onclick="toggleStatus(\''+s.id+'\',\'approved\')">'+(s.approved?'✓':'○')+' Final</button></div>').join('')+'</div></section>'}
function editSceneByIdStage(id,stage){const s=project().scenes.find(x=>x.id===id);if(!s)return;if(stage==='animation'){s.animation='pending';s.approved=false}else if(stage==='image'){s.image='pending';s.animation='pending';s.approved=false}save();render()}
...
function sceneMatchesFilter(s){
 ...
 if(sceneFilter==='ignored')return s.type==='Ignorar';
 if(sceneFilter==='pending')return s.type!=='Ignorar'&&(s.image!=='done'||s.animation!=='done'||!s.approved);
 return true
}
...
const dbName='animartoon-fs',storeName='handles',thumbStoreName='scene-thumbs',generatedStoreName='generated-assets';
function openFsDb(){return new Promise((resolve,reject)=>{const req=indexedDB.open(dbName,3);req.onupgradeneeded=()=>{if(!req.result.objectStoreNames.contains(storeName))req.result.createObjectStore(storeName);if(!req.result.objectStoreNames.contains(thumbStoreName))req.result.createObjectStore(thumbStoreName);if(!req.result.objectStoreNames.contains(generatedStoreName))req.result.createObjectStore(generatedStoreName)};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error)})}
...
async function saveGeneratedAsset(projectId,sceneId,kind,file){const db=await openFsDb(),value={blob:file,name:file.name,type:file.type,size:file.size,savedAt:Date.now()};return new Promise((resolve,reject)=>{const tx=db.transaction(generatedStoreName,'readwrite');tx.objectStore(generatedStoreName).put(value,projectId+':'+sceneId+':'+kind);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error)})}
async function loadGeneratedAsset(projectId,sceneId,kind){const db=await openFsDb();return new Promise((resolve,reject)=>{const req=db.transaction(generatedStoreName).objectStore(generatedStoreName).get(projectId+':'+sceneId+':'+kind);req.onsuccess=()=>resolve(req.result||null);req.onerror=()=>reject(req.error)})}
...
async function sendSceneToPreferredGenerator(sceneId){
 const p=project(),s=p.scenes.find(x=>x.id===sceneId);if(!s)return toast('Cena não encontrada');
 const imageStage=s.image!=='done',category=imageStage?'image':'animation',tools=(state.tools||defaultTools).filter(t=>t.category===category),tool=tools.find(t=>t.preferred)||tools[0];
 if(!tool)return toast('Nenhum gerador configurado');
 const prompt=imageStage?imagePrompt(s):animationPrompt(s);
 const opened=window.open(tool.url,'_blank');
 try{await navigator.clipboard.writeText(prompt)}catch(e){}
 try{
  let href='',downloadName='';
  if(!imageStage){
   const generated=await loadGeneratedAsset(p.id,s.id,'image');
   if(generated?.blob){href=URL.createObjectURL(generated.blob);downloadName=s.id+'_GENERATED_KEYFRAME'+((generated.type||'').includes('png')?'.png':'.jpg')}
  }
  if(!href){href=await loadSceneThumbnail(p.id,s.id);downloadName=s.id+'_SOURCE_REF.jpg'}
  if(href){
   const a=document.createElement('a');a.href=href;a.download=downloadName;document.body.appendChild(a);a.click();a.remove();
   if(href.startsWith('blob:'))setTimeout(()=>URL.revokeObjectURL(href),2000);
  }
 }catch(e){}
 s.lastPreparedAt=Date.now();s.lastPreparedTool=tool.name;s.lastPreparedStage=category;save();
 toast((opened?'Gerador aberto':'Permita pop-ups para abrir o gerador')+' • prompt copiado • referência preparada');
}
async function acceptGeneratedResult(sceneId,event){
 const file=event?.target?.files?.[0];if(!file)return;
 const p=project(),s=p.scenes.find(x=>x.id===sceneId);if(!s)return;
 const imageStage=s.image!=='done';
 if(imageStage&&!String(file.type||'').startsWith('image/'))return toast('Selecione a imagem gerada para esta cena');
 if(!imageStage&&String(file.type||'')&&!String(file.type||'').startsWith('video/'))return toast('Selecione o vídeo gerado para esta cena');
 try{
  if(imageStage){
   await saveGeneratedAsset(p.id,s.id,'image',file);
   s.image='done';s.imageVersion=(s.imageVersion||0)+1;s.generatedImageName=file.name;s.generatedImageAt=Date.now();
   toast('Imagem recebida. A próxima etapa será a animação.');
  }else{
   s.animation='done';s.animationVersion=(s.animationVersion||0)+1;s.generatedAnimationName=file.name;s.generatedAnimationAt=Date.now();
   toast('Animação recebida. Cena pronta para aprovação.');
  }
  save();render();
 }catch(e){toast('Não foi possível registrar o resultado gerado')}
}
...
function render(){const p=project();if(p?.transcriptAppliedAt&&p.contextRefinedVersion!==2){contextualRefineScenes(p,false);save()}nav();document.getElementById('pageTitle').textContent=state.page;document.getElementById('storagePill').textContent=p?.storage||'—';const mini=document.querySelector('.project-mini');if(mini)mini.innerHTML='<span>Projeto ativo</span><strong>'+esc(p?.name||'Nenhum')+'</strong><small>Sprint 2.4 • MVP 1.5</small>';const views={'Próxima ação':nextView,'Projetos':projectsView,'Importação':importView,'Produção':production,'Personagens':()=>assetsView('Personagens'),'Cenários':()=>assetsView('Cenários'),'Cenas':scenesView,'Mapa de arquivos':filemap,'Auditoria':audit,'Configurações':settings};document.getElementById('content').innerHTML=views[state.page]();configureAutoSync();if(state.page==='Cenas'||state.page==='Próxima ação')hydrateSceneThumbs()}
render();