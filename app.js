...
let sceneFilter='all';
const DEFAULT_ANALYZER_URL='https://analyzer-production-8860.up.railway.app';
...
function analyzeWithServer(){
 ...
 xhr.onload=()=>{try{
   ...
   p.analysisStats={step:'FFmpeg',threshold:data.threshold,thresholdLabel:'Servidor',visualCuts:data.visualCuts||0,technicalSplits:data.technicalSplits||0,cutoff:data.threshold,maxDiff:0,server:true,analyzedAt:Date.now()};
...
function analyzerPanel(p){
 ...
 return '<section class="card span2"><div class="section-title"><div><p class="eyebrow">ANALISADOR FFmpeg — SERVIÇO</p><h2>Processamento robusto fora do navegador</h2></div><span class="badge '+(s.ok?'ok':'warn')+'">'+(s.ok?'online':'não testado')+'</span></div><p class="muted">Use este modo quando a análise visual no navegador não conseguir extrair quadros. O vídeo local é enviado temporariamente ao serviço, analisado com FFmpeg e descartado após a resposta.</p><div class="form-grid"><label>URL do analisador<input id="analyzerUrl" value="'+esc(p.analyzerUrl||DEFAULT_ANALYZER_URL)+'"></label><div class="actions analyzer-actions"><button class="btn" onclick="testAnalyzer()">Testar serviço</button><button class="btn primary" onclick="analyzeWithServer()">Analisar arquivo com FFmpeg</button></div></div>'+(s.message?'<p class="muted note">Status: '+esc(s.message)+'</p>':'')+'</section>'
}
